# Beholding staged full-sermon canary
...
is the 1,888.1045-second *Beholding the Love of God* file. Tests use real-time
...
PYTHONPATH=.python-packages:backend python3 direct_asr_smoke.py \
  ...
  --fast \
  --json-output /tmp/beholding-prefix-810-asr.json
```
...
The one-minute staged WebSocket gate and historical Beholding operational gate
are passed. A short final-snapshot preflight should precede the remaining
staged Spirit/Blessed/full matrix,
