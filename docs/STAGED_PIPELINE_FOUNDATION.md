...
the first full sermon operational canary have passed. Beholding attempt 3
...
audience-experience gate. Spirit, Blessed, the complete staged matrix, browser
...
python direct_asr_smoke.py \
  --quiet \
  --json-output /tmp/pellera-direct-asr-smoke.json
```
...
python direct_asr_bridge_smoke.py \
  --quiet \
  --json-output /tmp/pellera-direct-asr-bridge-smoke.json
```
...
WebSocket integration, and first full staged sermon canary have since been
completed.
...
The WebSocket preflight and first full staged sermon operational run have
passed. Beholding attempt 3 consumed 1,888.1045 seconds, delivered all 646
...
6. Pass the one-minute preflight and short excerpts before running a complete
   sermon.
7. Pass the first full staged sermon operational canary with ordered output and
   one natural terminal.

Remaining promotion gates:

1. Run Spirit and Blessed and complete the staged three-sermon comparison
   matrix.
