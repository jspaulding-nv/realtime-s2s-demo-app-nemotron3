#!/usr/bin/env python3
"""Run the three long-form sermons and compare fixed/adaptive playback.

Each sermon is sent through the live Riva pipeline once per repeat. The
...
def sermon_slug(path: Path) -> str:
    name = path.stem.lower()
    if name.startswith("200108_spirit"):
        return "spirit"
    if name.startswith("blessed_self"):
        return "blessed"
    if "beholding_the_love_of_god" in name:
        return "beholding"
...
def build_manifest(
    *,
    ...
    sermons = []
    runs = []
    sermon_order: dict[str, int] = {}
    for file_index, path in enumerate(files):
        resolved = path.resolve()
        if not resolved.is_file():
            raise ExperimentError(f"sermon file not found: {path}")
        slug = sermon_slug(resolved)
        sermon_order[slug] = file_index
        sermons.append(
            {
                "slug": slug,
    ...
    return {
        ...
        "provenance": {
            "live_riva_trace_per_sermon_repeat": True,
...
def validate_resume_provenance(manifest: dict[str, Any]) -> None:
    ...
    for sermon in manifest.get("sermons", []):
        path = Path(sermon["audio_path"])
        if not path.is_absolute():
            path = REPOSITORY_ROOT / path
        if not path.is_file():
            raise ExperimentError(f"resume sermon file is missing: {path}")
        if path.stat().st_size != int(sermon["size_bytes"]):
            raise ExperimentError(f"resume sermon size differs: {path}")
        if sha256_file(path) != sermon["sha256"]:
            raise ExperimentError(f"resume sermon SHA-256 differs: {path}")
...
def experiment_lock(backend_url: str):
    ...
    lock_path = Path(tempfile.gettempdir()) / f"pellera-s2s-{lock_key}.lock"
...
async def capture_and_promote(
    ...
    prefix = f"repeat-{entry.get('repeat', 0):02d}-{entry.get('sermon', 'preflight')}-"
...
def write_analysis(run_dir: Path, manifest: dict[str, Any]) -> dict[str, Any]:
    ...
    expected = manifest["requested_repeats"] * len(manifest["sermons"])
...
def print_plan(run_dir: Path, manifest: dict[str, Any]) -> None:
    print("Three-sermon experiment plan")
    ...
    for entry in manifest["runs"]:
        print(
            f"    repeat {entry['repeat']:02d}: {entry['sermon']} "
            f"({entry['audio_path']})"
        )
        ...
        for index, entry in enumerate(manifest["runs"], start=1):
            ...
            if valid:
                ...
                print(
                    ...
                    f"{entry['sermon']} (checkpoint valid; skipped) ==="
                )
                continue
            ...
            try:
                ...
                write_manifest(run_dir, manifest)
                raise ExperimentError(
                    f"repeat {entry['repeat']:02d} {entry['sermon']} failed: {exc}"
                ) from exc
            entry["status"] = "completed"
...
def parse_args(argv: Sequence[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description=(
            "Capture all three sermons through live Riva and compare fixed/adaptive playback"
        )
    )
    parser.add_argument(
        "--backend",
    ...
    parser.add_argument(
        "--repeats",
        type=int,
        help="live Riva traces per sermon (new-run default: 1)",
    )
    parser.add_argument(
        "--skip-preflight",
        action="store_true",
...
def main(argv: Sequence[str] | None = None) -> int:
    ...
    files = [REPOSITORY_ROOT / path for path in TEST_FILES]
